...
RNFS.exists(setup_path).then(res => {
    if(!res){
        RNFS.writeFile(setup_path,'1@CPU','utf8').then(res => {
            ...
            global.variables = {ccTflite: ccTflite,numThread: 1,device: 'CPU',userToken: false,userId: '',ip: 'http://192.168.43.144:8080',account: '',name:'',signature: '',avatar: '',background: ''}
        })
    }else{
        RNFS.readFile(setup_path).then(res => {
            ...
            ccTflite.ccloadModel('my_model3.tflite','my_lables2.txt',Number(temp[0]),temp[1])
                .then(res => console.log(res))
            global.variables = {ccTflite: ccTflite,numThread: Number(temp[0]),device: temp[1],userToken: false,userId: '',ip: 'http://192.168.43.144:8080',account: '',name:'',signature: '',avatar: '',background: ''}
        })
    }
})
